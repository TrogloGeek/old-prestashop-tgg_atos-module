...
<div class="margin-form">
        <input type="text" name="advanced_controls" id="field_advanced_controls" value="{$advanced_controls|escape:'html':'UTF-8'}"  style="width: 500px;" {if !$bool_advanced_controls}disabled="disabled" {else}required="required" {/if}/>
        <br />
        <br />
        <label class="t clearfix" style="width: 500px; line-height: 1.4em; text-align: left;"><input type="checkbox" name="bool_advanced_controls" style="vertical-align: text-bottom;" onclick="{literal}if (!$(this).attr('checked')) {$('#field_advanced_controls').attr('disabled', 'disabled');} else {$('#field_advanced_controls').removeAttr('disabled');}{/literal}" value="1" {if $bool_advanced_controls} checked="checked"{/if}/>&nbsp;&nbsp;{l s='Enable advanced controls for the SIPS API calls' mod='tgg_atos'}</label>
        <span class="hint">{l s='See user guide for advanced controls against fraud provided by your bank' mod='tgg_atos'}</span>
</div>
<!-- CUSTOM PARAMS -->
<label for="field_custom_params"{if in_array('custom_params', $highlights.ADVANCED)} class="highlighted"{/if}>{l s='Custom params:' mod='tgg_atos'}&nbsp;</label>
<div class="margin-form">
        <input type="text" name="custom_params" id="field_custom_params" value="{$custom_params|escape:'html':'UTF-8'}"  style="width: 500px;" {if !$bool_custom_params}disabled="disabled" {else}required="required" {/if}/>
        <br />
        <br />
        <label class="t clearfix" style="width: 500px; line-height: 1.4em; text-align: left;"><input type="checkbox" name="bool_custom_params" style="vertical-align: text-bottom;" onclick="{literal}if (!$(this).attr('checked')) {$('#field_custom_params').attr('disabled', 'disabled');} else {$('#field_custom_params').removeAttr('disabled');}{/literal}" value="1" {if $bool_custom_params} checked="checked"{/if}/>&nbsp;&nbsp;{l s='Enable ability to append custom parameters in your SIPS API calls' mod='tgg_atos'}</label>
        <span class="hint">{l s='For advanced users only' mod='tgg_atos'}</span>
</div>
